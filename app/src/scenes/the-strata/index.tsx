...
const LADDER = [
  ...
  { name: "MOSAIC", gloss: "the cloud-free composite" },
  { name: "FEATURES", gloss: "entities the model extracted" },
  { name: "REGIONS", gloss: "entities grouped into areas" },
...
function Overlay({ result }: { result: SimResult }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "absolute", left: 44, top: 38, maxWidth: 330 }}>
        <div style={{ fontSize: 30, fontWeight: 700, letterSpacing: "-0.02em", lineHeight: 1.05 }}>
          The Strata
        </div>
        <div style={{ fontSize: 14.5, lineHeight: 1.5, marginTop: 13, opacity: 0.66 }}>
          One territory, {n} times over. The slab at the bottom is the world. Each slab above it
          is the same ground after somebody made it smaller.
        </div>
      </div>

      <div style={{ position: "absolute", left: 44, bottom: 34, maxWidth: 300 }}>
        <div style={{ fontSize: 13, lineHeight: 1.55, opacity: 0.5 }}>
          The sparks climbing between slabs are reports. A slab only holds so much, so the
          loudest get in and the rest burn out at the boundary in ember. Find a bright
          settlement on the ground, then look straight up.
        </div>
      </div>
      ...
      >
        ...
        <span style={{ opacity: 0.82 }}>
          {" "}people — never reach the top slab. Their columns stop where somebody stopped
          counting them.
        </span>
      </div>
      ...
      >
        ...
        <div style={{ fontSize: 12.5, lineHeight: 1.55, marginTop: 16, opacity: 0.5 }}>
          Nothing else moves — same ground, same people, same growth logic, same grids. Slide
...
register({
  ...
  title: "The Strata",
  question:
    "The same territory is represented five times over, each one smaller than the last. Which communities survive every step up, and which one disappears on the way?",
